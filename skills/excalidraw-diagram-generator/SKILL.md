...
### Arrow Element Format Error

**Common Mistake**: Using non-existent `type: "arrow"` with `start`/`end` properties
...
**Key Points**:
- Excalidraw does NOT have a `type: "arrow"` element
- Use `type: "line"` with `endArrowhead: "arrow"` for arrows
- `points` array uses relative coordinates: `[[x1, y1], [x2, y2], ...]`
- First point is typically `[0, 0]`, representing offset from element's `x, y`

### Required Top-level Fields

Ensure every `.excalidraw` file includes:
```json
{
  "type": "excalidraw",
  "version": 2,
...
Missing these fields will cause "Unable to load initial data" error.
