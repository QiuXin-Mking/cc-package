...
#### REPOMAP 规则 — 让 Claude 自动读懂代码库

在大项目中生成 `REPOMAP.md` 后，在项目 `CLAUDE.md` 中加入此规则，Claude 会自动在跨模块分析、架构探索时读取：
...
Read it when the task benefits from a map: broad exploration, "where does X
live", cross-module refactors, onboarding. Skip it for narrow lookups where
Grep or a known file path is faster.
```

> 实验数据：5 个真实需求 × 4 轮对话对照实验，跨模块分析场景省 **50-150%** 时间（[完整报告](docs/repomap-bench-report.md)）。
